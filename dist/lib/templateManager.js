...
export class TemplateManager {
    ...
    async createDefaultTemplates() {
        ...
        await fs.writeFile(config.paths.templates, JSON.stringify(defaultTemplates, null, 2), 'utf-8');
    ...
    updatePerformanceScore(id, newScore) {
        const template = this.templates.find(t => t.id === id);
        if (template) {
            template.performanceScore = Math.min(1, Math.max(0, newScore));
            // Save updated templates
            fs.writeFile(config.paths.templates, JSON.stringify(this.templates, null, 2), 'utf-8').catch(console.error);
        }
    }
}
