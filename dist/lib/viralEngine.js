...
export class ViralEngine {
    ...
    async generateVideo(options) {
        ...
        try {
            ...
            const processedClips = await this.videoAssembler.trimAndResizeClips(clipsToProcess, script.segments, config.video.width, config.video.height);
            ...
            if (options.musicStyle) {
                ...
                try {
                    finalAudioPath = await this.audioManager.addBackgroundMusic(mergedAudioPath, musicStyle, musicAddedPath, 0.15 // Low volume for background
                    );
                }
                catch (error) {
            ...
            const outputPath = await this.videoAssembler.assembleVideo(project, transitionedClips, finalAudioPath);
    ...
    async generateBatch(topics, templateId) {
        const results = [];
...
//# sourceMappingURL=viralEngine.js.map