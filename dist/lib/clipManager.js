...
export class ClipManager {
    pexels;
    pixabay;
    downloadedClips = new Map();
    constructor() {
        this.pexels = new PexelsAPI();
        this.pixabay = new PixabayAPI();
    }
    async findClipsForScript(segments) {
        const allClips = [];
    ...
    selectBestClip(clips, segment) {
        ...
        const scoredClips = clips.map(clip => {
            ...
            if (aspectRatio < 0.6)
                score += 3; // Strong portrait
            else if (aspectRatio < 1)
                score += 1; // Weak portrait
    ...
    async downloadClip(clip, outputDir) {
    ...
    async downloadAllClips(clips, outputDir) {
...
//# sourceMappingURL=clipManager.js.map